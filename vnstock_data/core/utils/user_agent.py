...
def get_headers(data_source='SSI',random_agent=True):
	B=data_source;B=B.upper();C=UserAgent(fallback='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36')
	if B=='VDS':A=VDS_HEADERS
	else:A=DEFAULT_HEADERS.copy()
	if random_agent:A[_C]=C.random
	else:A[_C]=C.chrome
	A.update(HEADERS_MAPPING_SOURCE.get(B,{}));return A
