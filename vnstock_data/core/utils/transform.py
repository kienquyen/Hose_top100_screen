...
def process_match_types(df,asset_type,source):
	...
	if asset_type=='stock'and(A[_B].eq(F).any()or A[_B].eq('').any()):
		...
		def H(day_df):
			...
			return B
		A=A.groupby(E,group_keys=_G).apply(H);A.drop(columns=[E],inplace=True)
	return A
def ohlc_to_df(data:Dict[str,Any],column_map:Dict[str,str],dtype_map:Dict[str,str],asset_type:str,symbol:str,source:str,interval:str='1D',floating:int=2,resample_map:Optional[Dict[str,str]]=_F)->pd.DataFrame:
	P='Asia/Ho_Chi_Minh';O='UTC';L=resample_map;K=asset_type;J=interval;I=source;H=column_map;G=data;F='close';E='low';D='high';C='open'
	if not G:raise ValueError('Input data is empty or not provided.')
	if I=='TCBS':A=pd.DataFrame(G);A.rename(columns=H,inplace=True)
	else:Q={A:H[A]for A in H.keys()if A in G};A=pd.DataFrame(G)[Q.keys()].rename(columns=H)
	R=[_A,C,D,E,F,_C];M=[B for B in R if B not in A.columns]
	...
	if _A in A.columns:
		...
		else:A[_A]=pd.to_datetime(A[_A],errors=_H)
	if K not in['index','derivative']:A[[C,D,E,F]]=A[[C,D,E,F]].div(1000)
	A[[C,D,E,F]]=A[[C,D,E,F]].round(floating)
	if L and J not in['1m','1D']:A=A.set_index(_A).resample(L[J]).agg({C:'first',D:'max',E:'min',F:'last',_C:'sum'}).reset_index()
	for(B,N)in dtype_map.items():
		if B in A.columns:
			if N=='datetime64[ns]'and hasattr(A[B],'dt')and A[B].dt.tz is not _F:
				A[B]=A[B].dt.tz_localize(_F)
...
def intraday_to_df(data:List[Dict[str,Any]],column_map:Dict[str,str],dtype_map:Dict[str,str],symbol:str,asset_type:str,source:str)->pd.DataFrame:
	...
	for B in(F,_C):
		if B in A.columns:
			A[B]=A[B].map(clean_numeric_string);A[B]=pd.to_numeric(A[B],errors=_H);J=A[B].isna().sum()
	...
	if _A in A.columns:
		...
		if C==_D:A[_A]=localize_timestamp(A[_A].astype(int),unit='s')
			...
			if':'in M and len(M)<=8:A[_A]=A[_A].apply(lambda x:datetime.combine(R,datetime.strptime(x,'%H:%M:%S').time())if isinstance(x,str)and':'in x else pd.NaT);A[_A]=localize_timestamp(A[_A],return_string=_G)
			else:
				A[_A]=pd.to_datetime(A[_A],format='%Y-%m-%d %H:%M:%S',errors=_H)
				if A[_A].dt.tz is _F:A[_A]=localize_timestamp(A[_A],return_string=_G)
	if _B in A.columns:A=process_match_types(A,G,C)
	if _A in A.columns:A=A.sort_values(_A)
	A=A.reset_index(drop=True);N={B:C for(B,C)in dtype_map.items()if B in A.columns and B!=_A}
	if N:A=A.astype(N)
